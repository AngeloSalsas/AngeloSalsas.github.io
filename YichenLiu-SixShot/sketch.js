...
function setup() {
  ...
  mySlider = createSlider() // gui example. use the .position method in the draw function
}
...
function draw() {
  ...
  if (mode == 1) {
    ...
    // your code here, so delete from here...
    let gameState = "start";
    let score = 0;
    let startTime = 0;
    let gameDuration = 60;
    ...
    function drawStartScreen() {
      ...
      fill(255);
      textSize(64);
      text("Pulse Aim", width / 2, height / 2 - 95);
      ...
      fill(255, 170);
      textSize(15);
      text("60 seconds • 6 targets • musical layers", width / 2, height / 2 + 40);

      drawButton(width / 2, height / 2 + 105, 190, 58, "START");
    }
    ...
    function drawEndScreen() {
      ...
      fill(255, 190);
      textSize(16);
      text("Phase 3 begins after 70 hits.", width / 2, height / 2 + 42);
    ...
    function mousePressed() {
      ...
      if (gameState === "start") {
        if (overButton(width / 2, height / 2 + 105, 190, 58)) startGame();
        return;
      }

      if (gameState === "end") {
        if (overButton(width / 2, height / 2 + 105, 220, 58)) startGame();
        return;
      }
